...
<div>
  <!-- header -->
  <div class="table-header invoice-table text-daisyBush lg:grid lg:p-6">
    <h3>Status</h3>
    <h3>Due Date</h3>
    <h3>ID #</h3>
    <h3>Client</h3>
  ...
  <!-- list of invioces -->
  <div
    class="invoice-table mb-3 grid grid-cols-invoiceTableMobile items-center gap-x-4 rounded-lg bg-white px-4 py-6 shadow-tableRow lg:grid-cols-invoiceTable lg:p-6"
  >
    <div class="status"><Tag className="ml-auto lg:ml-0" label="draft" /></div>
    <div class="dueDate text-sm lg:text-lg">12/12/20225</div>
    <div class="invoiceNr text-sm lg:text-lg">55555</div>
    <div class="client text-base font-bold lg:text-lg">fds</div>
    <div class="amount text-right font-mono text-sm lg:text-lg">$100.00</div>
    <div class="vievBtn hidden lg:block ">
      <a href="/" class=" text-pastelPurple hover:text-daisyBush"><Eye /></a>
    </div>
    <div class="moreBtn hidden lg:block">
...
<style lang="postcss">
  ...
  @media (min-width: 1024px) {
    .invoice-row {
      grid-template-areas: 'status dueDate invoiceNr client amount viewBtn moreBtn';
    }
  }
  .invoice-row .status {
    grid-area: status;
  ...
  /* .invoice-row .viewBtn, .invoice-row .moreBtn{
		@apply text-center;
	} */
</style>
